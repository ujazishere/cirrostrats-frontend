...
test("Gate Suggestions Click: C101", async ({ page }) => {
...
test("Gate Raw Submit: C102", async ({ page }) => {
  await navigateToDetailsPage({
    page,
...
test("Gate Raw Submit: A21", async ({ page }) => {
  await navigateToDetailsPage({
    page,
    navigationMethod: "raw",
    query: "A21",
  ...
  await expect(page.getByRole("heading", { name: "Gate A21" })).toBeVisible();
  await expect(page.getByText("Flight")).toBeVisible();
  await expect(page.getByText("Scheduled")).toBeVisible();
  await expect(page.locator("div.flight-row-card").first()).toBeVisible();
});
