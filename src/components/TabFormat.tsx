...
import { FlightData, NASData, CombinedWeatherData } from "../types";

interface TabFormatProps {
...
const getDefaultTab = (tabOrder: string[]): string => {
...
// ... (rest of your code)
  ...
  const hasWeatherData = (weatherData: any): boolean => {
  ...
  const [activeTab, setActiveTab] = useState(getDefaultTab(tabOrder));
  ...
  useEffect(() => {
    if (!tabOrder.includes(activeTab)) {
      setActiveTab(getDefaultTab(tabOrder));
    }
  }, [hasAltDepWeather, hasAltDestWeather, activeTab, tabOrder]);
