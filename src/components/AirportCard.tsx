...
const AirportCard = ({
  ...
  const [selectedDatisType, setSelectedDatisType] = useState<"DEP" | "ARR">(
    "DEP",
  );
   ...
   */
  const calculateMinutesAgoHHMMZ = (timeString: string): number | null => {
   ...
   * @returns {number} - Minutes ago from current UTC time
   */
  const calculateMinutesAgoDDHHMMZ = (timeString: string): number | null => {
    if (!timeString || typeof timeString !== "string") return null;
  ...
  const getSelectedDatisTimestamp = (): string => {
    if (hasCombinedDatis && datis?.combined) {
      return datis.combined.datis_ts || "";
    }
    const datisType = selectedDatisType.toLowerCase() as "arr" | "dep";
    return datis?.[datisType]?.datis_ts || "";
  };
  ...
  return (
    <div className="weather-container">
      {/* Search Input Component at the top with the same styling as combined.tsx | DO NOT DELETE THIS CODE */}
      {showSearchBar && (
      ...
      <NASDetails nasResponse={nasResponseAirport} title={getNASTitle()} />
        ...
        <div className="weather-card">
          ...
          <div className="card-body">
            <div className="data-content">
              <p
                style={{ lineHeight: "1.87083" }}
                dangerouslySetInnerHTML={{ __html: metar || "N/A" }}
              ></p>
            </div>
          </div>
        </div>
        ...
        <div className="weather-card">
          ...
          <div className="card-body">
            <div className="data-content">
              <p
                style={{ lineHeight: "1.87083" }}
                dangerouslySetInnerHTML={{ __html: taf || "N/A" }}
              ></p>
            </div>
          </div>
        </div>
